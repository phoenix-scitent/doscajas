...
Router.route('/sequence/:_id/edit', {
  ...
  template: 'sequence_form',
  data: function() {
    return Sequence.findOne({ _id: this.params._id });
  }
});
...
Router.route('/sequence/:_id/attempt', {
  name: 'sequence_attempt',
...
Router.route('/sequence/attempt', {
  name: 'sequence_attempt_test',
  template: 'sequence_attempt',
  layoutTemplate: 'layout2',
  data: function() {
    return null;
...
Router.route('/', function () {
  if(Meteor.user()){
    ...
    this.render('accountLogin');
    this.layout('blankLayout');
    // this blinks the boks view for a split second...
    // Router.go('login');
  }
