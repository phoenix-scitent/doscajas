...
Meteor.methods({
  ...
  createAttempt: function(sequenceId, user){
    ...
    var learner = Meteor.user()
    var source = Sequences.find({ _id: sequenceId }).fetch();

    attempt = {
      attempt: {
        parent: source._id
        user: learner._id,
        count: Sequences.find({ "attempt.parent": source._id, "attempt.user": learner._id }).count() + 1, //TODO: make this dynamic +1 for each new attempt passed in from last attempt
        score: 0,
        items: _.map(source.items, function(item){
          var data;

          if(item.type === 'measure'){
            data = Measures.find({ _id: item._id }).fetch();
          } else {
            data = Resources.find({ _id: item._id }).fetch();
          }
