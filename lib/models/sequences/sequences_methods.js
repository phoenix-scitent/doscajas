Meteor.methods({
  ...
  createAttempt: function(sequenceId, user){
    ...
    var originalSource = Sequences.findOne({ _id: sequenceId });
    ...
    if(isAdaptiveAttempt){
      source = _.last(Sequences.find({'attempt.original': sequenceId, 'attempt.user': user._id}).fetch()) || originalSource;
    } else {
    ...
    attempt = {
      ...
      parent: source._id,
      user: user._id,
      count: (isAdaptiveAttempt && source.attempt) ? (source.attempt.count + 1) : (Sequences.find({ "attempt.parent": source._id, "attempt.user": user._id }).count() + 1),
      score: 0,
      completed_at: null,
  ...
  selectAnswer: function(choice, measure_id, attempt_id) {
    var current_attempt = Sequences.findOne({_id: attempt_id});
    ...
    Meteor.promise("measure_activity_log", current_attempt.attempt.user, measure._id, current_attempt._id, measure.answers[choice - 1]).then(function(response) {
      // do nothing just be happy that it logged
    });

    return Sequences.update(attempt_id, {$set: {'attempt.items': current_attempt.attempt.items }}, function(){
  ...
  updateScore: function(attempt_id){
    ...
    Sequences.update(attempt_id, {$set: { 'attempt.items': adaptiveItemsMap, 'attempt.score': totalScore, 'attempt.topic_stats': tagProgress }});

    return totalScore;
  },
  removeSequence: function(attemptId){
    return Sequences.remove(attemptId);
  },
