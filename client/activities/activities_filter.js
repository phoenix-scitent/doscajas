Template.activities_filter.rendered = function(){
  ...
  Session.set('current_activity_filter', BOK.current()._id);

  $("#activities-filter").selectize({
    ...
    items: [ Session.get('current_activity_filter') ]
  });

  var filterSelectizeAPI = $('#activities-filter')[0].selectize;

  filterSelectizeAPI.on("item_add", function(value, $item){
    var _id = value;

    Session.set("current_activity_filter", _id);
