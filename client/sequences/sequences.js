...
Template.sequences.helpers({
  ...
  sequence_sections: function() {
    ...
    var sequences = _.map(SEQUENCES.getAvailable({ bokId: BOK.current()._id, userId: Meteor.userId(), tags: Session.get("current_sequence_filter") }).fetch(), function(sequence){
