Template.courses_filter.rendered = function(){
  ...
  Session.set('current_course_filter', BOK.current()._id);

  $("#courses-filter").selectize({
    ...
    items: [ Session.get('current_course_filter') ]
  });

  var filterSelectizeAPI = $('#courses-filter')[0].selectize;

  filterSelectizeAPI.on("item_add", function(value, $item){
    var _id = value;

    Session.set("current_course_filter", _id);
